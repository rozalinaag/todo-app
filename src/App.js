...
function App() {
  ...
  const [lists, setLists] = useState(
    DB.lists.map(item =>{
    item.color = DB.colors.filter(color => color.id === item.colorId)[0].name; //извлекаем название цвета из json-файла через две таблицы
    return item;
  }));

  const onAddList =(obj) => {
    const newList = [
      //с помощью конкатинации добавляем в бд массив
      ...lists, obj ];
      setLists(newList);
  };
